...
#### Top5 — AI 기반 알림 생성 (예정, Top4 완료 후)

채팅에서 자연어로 "삼성전자 320,000원 넘으면 알려줘" 같은 표현을 보냈을 때
LLM이 직접 알림 룰을 등록할 수 있게 한다. Top4에서 만든 alert 백엔드/워커/
텔레그램 통신은 그대로 재사용, **LLM tool-use 계층**만 추가.

작업 범위:
- `LLMClient.ask()` 시그니처에 `tools` 옵셔널 파라미터 추가 (Anthropic + Gemini
  모두 표준 지원). multi-turn tool dispatching 처리 — 백엔드가 tool_use 응답을
  보면 실제 함수 실행 후 결과를 LLM에 다시 던져 최종 답변을 받음.
- 노출 툴: `create_alert_rule`, `list_alerts`, `delete_alert`
  · 함수 구현은 Top4의 service 레이어 재사용 (UI 폼 → 같은 함수 호출 구조라
    코드 재사용 100%)
- **확인 단계 필수**: LLM이 `create_alert_rule` 호출 직후 바로 DB INSERT 하지 말 것.
  ChatPanel에 "확인 카드" UI를 띄워 사용자가 명시적으로 [확인] 누를 때만 실행.
  → 환각으로 잘못된 룰이 만들어져도 단순한 어색한 제안일 뿐, 데이터 손상 X.
- 안전장치:
  · 한 채팅 응답에서 생성 가능 룰 개수 cap (예: 3개)
  · 종목 코드는 LLM이 추측하지 않고 컨텍스트의 instrument에서 가져옴
  · 임계값이 현재가에서 너무 멀거나 가까우면 LLM에 재확인 시키는 system prompt

Top4의 D(수동 UI)가 fallback 역할 — LLM 통합이 망가져도 사용자가 직접 룰을
손볼 수 있어야 함.
